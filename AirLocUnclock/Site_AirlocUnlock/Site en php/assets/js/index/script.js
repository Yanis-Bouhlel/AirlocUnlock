//const ApiUrl = 'http://51.210.151.13/btssnir/projets2023/airlocunlock/api-airlocunlock/reservations.php'
...
function récupAPI(url) {
  /* fetch(url)
    ...
    var request = $.ajax({
            ...
            method: "GET",
            success: (data) =>{
                affichagecard(data.reservations);
            }
        })
}
...
function affichagecard(data) {
    let i=1;
    data.forEach(value => {
        const { nom, date, photo1, photo2, photo3, note, prix, id, lng, lat,ClientTapkey } = value;
        //creation d'une card maison 
        const vignette = document.createElement('div');
        vignette.setAttribute('id',i);
        vignette.classList.add('maison');
        vignette.innerHTML = `
<div class="col-md-4">
    <a href="PageReserv.html?id=${id}">
      <div class="card card maison" style="border-style: solid;">
        <div class="card-body card maison" style="border-style: none;">
          <div id="carousel-${i}" class="carousel slide" data-bs-ride="false" style="width: 300px;height: 300px;margin-top: -18px;">
            ...
            <div>
                <a class="carousel-control-prev" href="#carousel-${i}" role="button" data-bs-slide="prev"><span class="visually-hidden">Previous<span><i class="icon-arrow-right" style="color: rgb(0,0,0);padding-bottom: 0px;padding-top: 0px;font-size: 21px;"></i></span></span><span style="border-radius: 72px;background: #ffffff;font-size: 18px;border-width: 26px;padding: 0px;padding-top: 3px;padding-bottom: 2px;padding-left: 2px;padding-right: 2px;margin-top: 4px;height: 28.7969px;width: 32px;"><span><i class="icon-arrow-left" style="color: rgb(0,0,0);padding-bottom: 0px;padding-top: 0px;font-size: 21px;"></i></span></span></a><a class="carousel-control-next" href="#carousel-${i}" role="button" data-bs-slide="next"><span class="visually-hidden">Next</span><span></span><span style="border-radius: 72px;background: #ffffff;font-size: 18px;border-width: 26px;padding: 0px;padding-top: 3px;padding-bottom: 2px;padding-left: 2px;padding-right: 2px;margin-top: 4px;height: 28.7969px;width: 32px;"><span><i class="icon-arrow-right" style="color: rgb(0,0,0);padding-bottom: 0px;padding-top: 0px;font-size: 21px;"></i></span></span></a>
              </div>
            ...
            <h4 class="card-title" style="font-size: 17px;font-weight: bold;width: 254px;text-align: left;">${nom} <span><i class="fa fa-star"></i>${note}</span></h4>
